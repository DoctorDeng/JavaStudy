...
public class MergeSort extends BasicSort {
    ...
    private void mergeSortByLoop(int[] array, int start, int end) {
        ...
        for (int range = 1; range < length; range = range + range) {
            for (int _start = start; _start <= end; _start += range + range) {
                ...
                int _mid = _start + range -1;
                if (_mid > end) {
                    _mid = end;
                }
                mergeNotCreateTempArray(array, temp, _start, _mid, _end);
                //merge(array, _start, _mid, _end);
            }
        }
    }

    /**
     * 将数组下标 start ~ mid 和 mid + 1 ~ end 两范围内的数据进行归并操作，每次归并都会创建临时数组.
     * @param array 待归并数组
     * @param start 起始下标
     * @param mid   中间下标
     * @param end   结尾下标
    ...
    /**
     * 将数组下标 start ~ mid 和 mid + 1 ~ end 两范围内的数据进行归并操作，使用传入的临时数组进行归并操作.
     * @param array 待归并数组
     ...
     * @param end   结尾下标
     */
    private void mergeNotCreateTempArray(int[] array, int[] temp, int start, int mid, int end) {
        int leftCursor = start;
        int rightCursor = mid + 1;
        // 1. 复制待排序数据到临时数组中
        System.arraycopy(array, start, temp, start, end - start + 1);
        // 2. 进行归并操作并直接将数据复制回原数组
